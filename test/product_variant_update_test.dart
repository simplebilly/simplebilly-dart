//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:simplebilly_api/api.dart';
import 'package:test/test.dart';

// tests for ProductVariantUpdate
void main() {
  // final instance = ProductVariantUpdate();

  group('test ProductVariantUpdate', () {
    // String barcode
    test('to test the property `barcode`', () async {
      // TODO
    });

    // String imageLink
    test('to test the property `imageLink`', () async {
      // TODO
    });

    // bool isActive
    test('to test the property `isActive`', () async {
      // TODO
    });

    // Human-readable variant label, e.g. \"Red / M\".
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Option name → value map, e.g. `{\"Color\": \"Red\", \"Size\": \"M\"}`.
    // Object optionValues
    test('to test the property `optionValues`', () async {
      // TODO
    });

    // Explicit override price for this variant (takes precedence over parent price + delta).
    // String price
    test('to test the property `price`', () async {
      // TODO
    });

    // Price adjustment relative to the parent product's `default_price`.
    // String priceDelta
    test('to test the property `priceDelta`', () async {
      // TODO
    });

    // The parent product this variant belongs to. References the product entity.
    // String productId
    test('to test the property `productId`', () async {
      // TODO
    });

    // Variant-specific SKU (must be unique per tenant).
    // String sku
    test('to test the property `sku`', () async {
      // TODO
    });

    // Variant-level stock (optional — may be tracked on the parent only).
    // int stockQuantity
    test('to test the property `stockQuantity`', () async {
      // TODO
    });


  });

}

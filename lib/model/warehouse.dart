...
class Warehouse {
  ...
  @override
  String toString() => 'Warehouse[addressCity=$addressCity, addressCountry=$addressCountry, addressStreet=$addressStreet, addressZip=$addressZip, binLocations=$binLocations, code=$code, isActive=$isActive, isDefault=$isDefault, name=$name, notes=$notes]';
  ...
  static Warehouse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'code'), 'Required key "Warehouse[code]" is missing from JSON.');
        assert(json[r'code'] != null, 'Required key "Warehouse[code]" has a null value in JSON.');
        assert(json.containsKey(r'name'), 'Required key "Warehouse[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "Warehouse[name]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<Warehouse> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Warehouse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Warehouse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Warehouse> mapFromJson(dynamic json) {
    final map = <String, Warehouse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Warehouse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<Warehouse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Warehouse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Warehouse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

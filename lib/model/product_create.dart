...
class ProductCreate {
  /// Returns a new [ProductCreate] instance.
  ProductCreate({
    this.availability,
    this.barcode,
    this.brand,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ProductCreate &&
  ...
  /// Returns a new [ProductCreate] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ProductCreate? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "ProductCreate[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "ProductCreate[name]" has a null value in JSON.');
        assert(json.containsKey(r'productCode'), 'Required key "ProductCreate[productCode]" is missing from JSON.');
        assert(json[r'productCode'] != null, 'Required key "ProductCreate[productCode]" has a null value in JSON.');
        assert(json.containsKey(r'sku'), 'Required key "ProductCreate[sku]" is missing from JSON.');
        assert(json[r'sku'] != null, 'Required key "ProductCreate[sku]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<ProductCreate> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ProductCreate>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ProductCreate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ProductCreate> mapFromJson(dynamic json) {
    final map = <String, ProductCreate>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ProductCreate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ProductCreate-objects as value to a dart map
  static Map<String, List<ProductCreate>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ProductCreate>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ProductCreate.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

...
enum Severity {
  ...
  /// Returns the instance of [Severity] that was successfully decoded
  /// from the passed [value] on success, null otherwise.
  static Severity? fromJson(dynamic value) => SeverityTypeTransformer().decode(value);

  /// Returns a [List] containing instances of [Severity]
  /// that were successfully decoded from the passed [JSON][json].
  static List<Severity> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Severity>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Severity.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
...
class SeverityTypeTransformer {
  ...
  /// Encodes this enum as a value suitable for JSON.
  String encode(Severity data) => data._value;
  ...
  Severity? decode(dynamic data, {bool allowNull = true}) {
    if (data is Severity) {
      return data;
    }
    if (data != null) {
      switch (data) {
        ...
        case r'Critical': return Severity.critical;
        default:
          if (!allowNull) {

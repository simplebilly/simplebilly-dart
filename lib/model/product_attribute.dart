...
class ProductAttribute {
  ...
  @override
  String toString() => 'ProductAttribute[isFilterable=$isFilterable, name=$name, position=$position, productId=$productId, unit=$unit, value=$value]';
  ...
  static ProductAttribute? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "ProductAttribute[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "ProductAttribute[name]" has a null value in JSON.');
        assert(json.containsKey(r'productId'), 'Required key "ProductAttribute[productId]" is missing from JSON.');
        assert(json[r'productId'] != null, 'Required key "ProductAttribute[productId]" has a null value in JSON.');
        assert(json.containsKey(r'value'), 'Required key "ProductAttribute[value]" is missing from JSON.');
        assert(json[r'value'] != null, 'Required key "ProductAttribute[value]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<ProductAttribute> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ProductAttribute>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ProductAttribute>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ProductAttribute.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

...
class UserProfile {
  ...
  @override
  String toString() => 'UserProfile[createdAt=$createdAt, email=$email, emailVerified=$emailVerified, firstName=$firstName, fullName=$fullName, id=$id, lastName=$lastName]';
  ...
  static UserProfile? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        ...
        assert(json.containsKey(r'email_verified'), 'Required key "UserProfile[email_verified]" is missing from JSON.');
        assert(json[r'email_verified'] != null, 'Required key "UserProfile[email_verified]" has a null value in JSON.');
        assert(json.containsKey(r'first_name'), 'Required key "UserProfile[first_name]" is missing from JSON.');
        assert(json[r'first_name'] != null, 'Required key "UserProfile[first_name]" has a null value in JSON.');
        assert(json.containsKey(r'full_name'), 'Required key "UserProfile[full_name]" is missing from JSON.');
        assert(json[r'full_name'] != null, 'Required key "UserProfile[full_name]" has a null value in JSON.');
        ...
        assert(json.containsKey(r'last_name'), 'Required key "UserProfile[last_name]" is missing from JSON.');
        assert(json[r'last_name'] != null, 'Required key "UserProfile[last_name]" has a null value in JSON.');
        return true;
      }());

...
class Voucher {
  ...
  static Voucher? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'currency'), 'Required key "Voucher[currency]" is missing from JSON.');
        assert(json[r'currency'] != null, 'Required key "Voucher[currency]" has a null value in JSON.');
        assert(json.containsKey(r'voucherDate'), 'Required key "Voucher[voucherDate]" is missing from JSON.');
        assert(json[r'voucherDate'] != null, 'Required key "Voucher[voucherDate]" has a null value in JSON.');
        assert(json.containsKey(r'voucherStatus'), 'Required key "Voucher[voucherStatus]" is missing from JSON.');
        assert(json[r'voucherStatus'] != null, 'Required key "Voucher[voucherStatus]" has a null value in JSON.');
        assert(json.containsKey(r'voucherType'), 'Required key "Voucher[voucherType]" is missing from JSON.');
        assert(json[r'voucherType'] != null, 'Required key "Voucher[voucherType]" has a null value in JSON.');
        return true;
      }());

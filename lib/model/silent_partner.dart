...
class SilentPartner {
  /// Returns a new [SilentPartner] instance.
  SilentPartner({
    this.contractDate,
    this.einlage,
    this.gewinnquotePct,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SilentPartner &&
  ...
  @override
  String toString() => 'SilentPartner[contractDate=$contractDate, einlage=$einlage, gewinnquotePct=$gewinnquotePct, gewinnvortrag=$gewinnvortrag, instrumentType=$instrumentType, kestPflichtig=$kestPflichtig, name=$name, notes=$notes, verlustVerrechnungskonto=$verlustVerrechnungskonto, verlustbeteiligung=$verlustbeteiligung]';
  ...
  /// Returns a new [SilentPartner] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SilentPartner? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'instrumentType'), 'Required key "SilentPartner[instrumentType]" is missing from JSON.');
        assert(json[r'instrumentType'] != null, 'Required key "SilentPartner[instrumentType]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<SilentPartner> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SilentPartner>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SilentPartner.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SilentPartner> mapFromJson(dynamic json) {
    final map = <String, SilentPartner>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SilentPartner.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SilentPartner-objects as value to a dart map
  static Map<String, List<SilentPartner>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SilentPartner>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SilentPartner.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

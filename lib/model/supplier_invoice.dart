...
class SupplierInvoice {
  /// Returns a new [SupplierInvoice] instance.
  SupplierInvoice({
    this.currency,
    this.goodsReceiptId,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SupplierInvoice &&
  ...
  @override
  String toString() => 'SupplierInvoice[currency=$currency, goodsReceiptId=$goodsReceiptId, invoiceDate=$invoiceDate, invoiceNumber=$invoiceNumber, lineItems=$lineItems, notes=$notes, purchaseOrderId=$purchaseOrderId, status=$status, supplierContactId=$supplierContactId, supplierName=$supplierName, totalGrossAmount=$totalGrossAmount, totalNetAmount=$totalNetAmount]';
  ...
  /// Returns a new [SupplierInvoice] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SupplierInvoice? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'invoiceDate'), 'Required key "SupplierInvoice[invoiceDate]" is missing from JSON.');
        assert(json[r'invoiceDate'] != null, 'Required key "SupplierInvoice[invoiceDate]" has a null value in JSON.');
        assert(json.containsKey(r'invoiceNumber'), 'Required key "SupplierInvoice[invoiceNumber]" is missing from JSON.');
        assert(json[r'invoiceNumber'] != null, 'Required key "SupplierInvoice[invoiceNumber]" has a null value in JSON.');
        assert(json.containsKey(r'lineItems'), 'Required key "SupplierInvoice[lineItems]" is missing from JSON.');
        assert(json.containsKey(r'status'), 'Required key "SupplierInvoice[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "SupplierInvoice[status]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<SupplierInvoice> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SupplierInvoice>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SupplierInvoice.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SupplierInvoice> mapFromJson(dynamic json) {
    final map = <String, SupplierInvoice>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SupplierInvoice.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SupplierInvoice-objects as value to a dart map
  static Map<String, List<SupplierInvoice>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SupplierInvoice>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SupplierInvoice.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

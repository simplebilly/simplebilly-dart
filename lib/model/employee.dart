...
class Employee {
  ...
  @override
  String toString() => 'Employee[address=$address, backupEmployeeId=$backupEmployeeId, bic=$bic, city=$city, country=$country, createdAt=$createdAt, dateOfBirth=$dateOfBirth, deletedAt=$deletedAt, departmentId=$departmentId, email=$email, firstName=$firstName, gender=$gender, hireDate=$hireDate, hourlyCost=$hourlyCost, iban=$iban, id=$id, jobTitle=$jobTitle, lastLogin=$lastLogin, lastName=$lastName, lastUpdated=$lastUpdated, monthlySalary=$monthlySalary, phone=$phone, state=$state, status=$status, tenantId=$tenantId, updatedAt=$updatedAt, userId=$userId, weeklyHours=$weeklyHours, zip=$zip]';

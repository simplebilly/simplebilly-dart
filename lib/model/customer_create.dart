...
class CustomerCreate {
  ...
  @override
  String toString() => 'CustomerCreate[address=$address, contactPerson=$contactPerson, email=$email, externalOrderNumber=$externalOrderNumber, name=$name, paymentGracePeriodDays=$paymentGracePeriodDays, phone=$phone, vatId=$vatId]';
  ...
  static CustomerCreate? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "CustomerCreate[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "CustomerCreate[name]" has a null value in JSON.');
        return true;
      }());

...
class SilentPartnerUpdate {
  /// Returns a new [SilentPartnerUpdate] instance.
  SilentPartnerUpdate({
    this.contractDate,
    this.einlage,
    this.gewinnquotePct,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SilentPartnerUpdate &&
  ...
  @override
  String toString() => 'SilentPartnerUpdate[contractDate=$contractDate, einlage=$einlage, gewinnquotePct=$gewinnquotePct, gewinnvortrag=$gewinnvortrag, instrumentType=$instrumentType, kestPflichtig=$kestPflichtig, name=$name, notes=$notes, verlustVerrechnungskonto=$verlustVerrechnungskonto, verlustbeteiligung=$verlustbeteiligung]';
  ...
  /// Returns a new [SilentPartnerUpdate] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SilentPartnerUpdate? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<SilentPartnerUpdate> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SilentPartnerUpdate>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SilentPartnerUpdate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SilentPartnerUpdate> mapFromJson(dynamic json) {
    final map = <String, SilentPartnerUpdate>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SilentPartnerUpdate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SilentPartnerUpdate-objects as value to a dart map
  static Map<String, List<SilentPartnerUpdate>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SilentPartnerUpdate>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SilentPartnerUpdate.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

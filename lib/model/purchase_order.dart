...
class PurchaseOrder {
  ...
  @override
  String toString() => 'PurchaseOrder[currency=$currency, deliveryAddress=$deliveryAddress, expectedDeliveryDate=$expectedDeliveryDate, lineItems=$lineItems, notes=$notes, orderDate=$orderDate, poNumber=$poNumber, status=$status, supplierContactId=$supplierContactId, supplierName=$supplierName, totalGrossAmount=$totalGrossAmount, totalNetAmount=$totalNetAmount]';
  ...
  /// Returns a new [PurchaseOrder] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PurchaseOrder? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'orderDate'), 'Required key "PurchaseOrder[orderDate]" is missing from JSON.');
        assert(json[r'orderDate'] != null, 'Required key "PurchaseOrder[orderDate]" has a null value in JSON.');
        assert(json.containsKey(r'poNumber'), 'Required key "PurchaseOrder[poNumber]" is missing from JSON.');
        assert(json[r'poNumber'] != null, 'Required key "PurchaseOrder[poNumber]" has a null value in JSON.');
        assert(json.containsKey(r'status'), 'Required key "PurchaseOrder[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "PurchaseOrder[status]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<PurchaseOrder> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PurchaseOrder>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PurchaseOrder.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PurchaseOrder> mapFromJson(dynamic json) {
    final map = <String, PurchaseOrder>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PurchaseOrder.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PurchaseOrder-objects as value to a dart map
  static Map<String, List<PurchaseOrder>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PurchaseOrder>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PurchaseOrder.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
